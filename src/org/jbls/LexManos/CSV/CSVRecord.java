	...
	public String getString(String name) throws IndexOutOfBoundsException
	{
		if (!mValues.containsKey(name.toLowerCase()))
			throw new IndexOutOfBoundsException(String.format(
					"CSV Missing index %s", name));
		
		return mValues.get(name.toLowerCase());
	}
	
	public int getInt(String name) throws IndexOutOfBoundsException,
			NumberFormatException
	{
		return Integer.parseInt(getString(name));
	}
	
	public double getDouble(String name) throws IndexOutOfBoundsException,
			NumberFormatException
	{
