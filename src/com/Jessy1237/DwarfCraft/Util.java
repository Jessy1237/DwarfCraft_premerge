...
public class Util
{
	...
	public ItemStack parseItem(String info)
	{
		...
		} catch (NumberFormatException e)
		{
			...
			if (mat == null)
			{
				System.out.println("DC ERROR: Could not parse material: "
						+ info);
				return null;
			}
			item = mat.getId();
			...
			case WOOL:
				...
				switch (item.getData().getData())
				{
					...
					case 15:
						return "Ink Sac";
					default:
						return String.format("Unknown Dye(%d)", item
								.getData().getData());
				}
			case DOUBLE_STEP:
				if (checkEquivalentBuildBlocks(item.getTypeId(), -1) != null)
	...
	{
		if ((ID >= 256 && ID <= 258) || (ID >= 267 && ID <= 279)
	...
	public ArrayList<Integer> checkEquivalentBuildBlocks(int ID, int compareID)
	{
		...
		for (ArrayList<Integer> blocks : plugin.getConfigManager()
				.getBlockGroups().values())
		{
	...
	{
		String race = player.getRace().substring(0, 1).toUpperCase()
				+ player.getRace().substring(1);
		return plugin.getConfigManager().getPrefix()
				.replace("%racename%", race);
	}
	
	public String getPlayerPrefix(String race)
	{
		String raceStr = race.substring(0, 1).toUpperCase()
				+ race.substring(1);
		return plugin.getConfigManager().getPrefix()
				.replace("%racename%", raceStr);
	}
	...
	public enum FoodLevel
	{
		...
		APPLE(260, 4, 2.4f), BAKED_POTATO(393, 5, 7.2f), BREAD(297, 5, 6f), CAKE(
				92, 2, 2f), CARROT(391, 3, 4.8f), COOKED_CHICKEN(366, 6, 7.2f), COOKED_FISH(
				350, 5, 6f), COOKED_MUTTON(424, 6, 9.6f), COOKED_PORKCHOP(
				320, 8, 12.8f), COOKED_RABBIT(412, 5, 6f), COOKIE(357, 2,
				0.4f), GOLDEN_APPLE(322, 4, 9.6f), GOLDEN_CARROT(396, 6,
				14.4f), MELON(360, 2, 1.2f), MUSHROOM_STEW(282, 6, 7.2f), POISONOUS_POTATO(
				394, 2, 1.2f), POTATO(392, 1, 0.6f), PUMPKIN_PIE(400, 8, 4.8f), RABBIT_STEW(
				413, 10, 12f), RAW_BEEF(363, 3, 1.8f), RAW_CHICKEN(365, 2,
				1.2f), RAW_FISH(349, 2, 0.4f), RAW_MUTTON(423, 2, 1.2f), RAW_PORKCHOP(
				319, 3, 1.8f), RAW_RABBIT(411, 2, 1.8f), ROTTEN_FLESH(367, 4,
				0.8f), SPIDER_EYE(375, 2, 3.2f), STEAK(364, 8, 12.8f);
