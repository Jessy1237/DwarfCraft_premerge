...
public final class DwarfTrainer
{
	...
	public void printLeftClick(Player player)
	{
		...
		} else
		{
			System.out
					.println(String
							.format("[DC] Error: Greeter %s has no left click message. Check your configuration file for message ID %d",
									getUniqueId(), getMessage()));
		}
		return;
	}
	...
	public void trainSkill(DCPlayer dCPlayer)
	{
		...
		if (skill == null)
		{
			plugin.getOut().sendMessage(player,
					"&cYour race doesn't have this skill!", tag);
			setWait(false);
			return;
		}
						...
						.contains(skill.getId()))
		{
			plugin.getOut().sendMessage(
					player,
					"&cYour race doesn't specialize in this skill! Max level is ("
							+ plugin.getConfigManager().getRaceLevelLimit()
							+ ")!");
			setWait(false);
			return;
		}
		...
		if (skill.getLevel() >= plugin.getConfigManager().getMaxSkillLevel())
		{
			plugin.getOut().sendMessage(
					player,
					"&cYour skill is max level ("
							+ plugin.getConfigManager().getMaxSkillLevel()
							+ ")!", tag);
			setWait(false);
			return;
		}
		...
		if (skill.getLevel() < getMinSkill())
		{
			plugin.getOut()
					.sendMessage(
							player,
							"&cI can't teach a low level like you, find a lower level trainer",
							tag);
			setWait(false);
			return;
		}
		...
		for (ItemStack costStack : trainingCostsToLevel)
		{
			...
			if (costStack.getAmount() == 0)
			{
				plugin.getOut().sendMessage(
						player,
						String.format("&aNo more &2%s &ais needed", plugin
								.getUtil().getCleanName(costStack)), tag);
				continue;
			}
			if (!player.getInventory().contains(costStack.getTypeId()))
			{
				if (plugin.getUtil().checkEquivalentBuildBlocks(
						costStack.getTypeId(), -1) != null)
				{
					ArrayList<Integer> i = plugin.getUtil()
							.checkEquivalentBuildBlocks(
									costStack.getTypeId(), -1);
					...
					if (!contains)
					{
						...
						plugin.getOut()
								.sendMessage(
										player,
										String.format(
												"&cAn additional &2%d %s &cis required",
												costStack.getAmount(),
												plugin.getUtil()
														.getCleanName(
																costStack)),
				...
				} else
				{
					...
					plugin.getOut().sendMessage(
							...
							String.format(
									"&cAn additional &2%d %s &cis required",
									costStack.getAmount(), plugin.getUtil()
											.getCleanName(costStack)), tag);
					continue;
				}
			}
			...
			for (ItemStack invStack : player.getInventory().getContents())
			{
				if (invStack == null)
					continue;
				if ((invStack.getTypeId() == costStack.getTypeId() && (invStack
						.getDurability() == costStack.getDurability() || (plugin
						.getUtil().isTool(invStack.getTypeId()) && invStack
						.getDurability() == invStack.getType()
						.getMaxDurability())))
						|| plugin.getUtil().checkEquivalentBuildBlocks(
								invStack.getTypeId(), costStack.getTypeId()) != null)
				{
					deposited = true;
					...
					if (costStack.getType()
							.equals(skill.Item1.Item.getType()))
					{
						skill.setDeposit1(skill.getDeposit1() + delta);
					} else if (costStack.getType().equals(
							skill.Item2.Item.getType()))
					{
			...
			} else
			{
				plugin.getOut().sendMessage(
						player,
						String.format(
								"&cAn additional &2%d %s &c is required",
								costStack.getAmount(), plugin.getUtil()
										.getCleanName(costStack)), tag);
				hasMats = false;
				deposited = true;
		...
		if (hasMats)
		{
			...
			plugin.getServer()
					.getPluginManager()
					.callEvent(
							new DwarfCraftLevelUpEvent(dCPlayer, this, skill));
			plugin.getOut()
					.sendMessage(player, "&6Training Successful!", tag);
		}
