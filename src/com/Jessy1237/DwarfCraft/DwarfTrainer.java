...
public final class DwarfTrainer {
	...
	public void lookAt(Player p, DwarfTrainer d) {
		...
		Location npcLoc = ((LivingEntity) d.getEntity().getEntity().getBukkitEntity()).getEyeLocation();
		double xDiff = l.getX() - npcLoc.getX();
		double yDiff = l.getY() - npcLoc.getY();
		double zDiff = l.getZ() - npcLoc.getZ();
		...
		plugin.getDataManager().updateTrainerLocation(d, (float)(newYaw - 90), (float)newPitch);
		return;
	}
