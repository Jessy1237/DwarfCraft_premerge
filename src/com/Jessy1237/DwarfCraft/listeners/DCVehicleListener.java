...
public class DCVehicleListener implements Listener
{
					...
					if (effect.getEffectType() == EffectType.VEHICLEDROP)
					{
						...
						DwarfCraftEffectEvent ev = new DwarfCraftEffectEvent(
								...
								new ItemStack[] { new ItemStack(
										Material.BOAT, 1) },
								new ItemStack[] { drop }, null, null, null,
								null, event.getVehicle().getVehicle(), null,
								null);
	...
	public void onVehicleEnter(VehicleEnterEvent event)
	{
		...
		if (!(event.getVehicle() instanceof CraftBoat))
			return;
		plugin.getDataManager().addVehicle(
				new DwarfVehicle(event.getVehicle()));
		if (DwarfCraft.debugMessagesThreshold < 6)
