...
public class DCPlayerListener implements Listener
{
	...
	public void onPlayerJoin(PlayerJoinEvent event)
	{
		...
		{
			if (!plugin.getChat().getPlayerPrefix(player)
					.contains(plugin.getUtil().getPlayerPrefix(data)))
				plugin.getChat().setPlayerPrefix(
						player,
						plugin.getUtil().getPlayerPrefix(data) + " "
	...
	public void onPlayerItemConsume(PlayerItemConsumeEvent event)
	{
		...
		for (Skill s : skills.values())
		{
			...
			{
				if (e.getEffectType() == EffectType.EAT
						&& e.checkInitiator(item))
				{
					int foodLevel = plugin.getUtil().randomAmount(
							(e.getEffectAmount(dcPlayer)));
					
					DwarfCraftEffectEvent ev = new DwarfCraftEffectEvent(
							dcPlayer, e, null, null, lvl, foodLevel, null,
							null, null, null, item);
					...
					player.setFoodLevel((player.getFoodLevel() - lvl)
							+ foodLevel);
				}
			}
		}
				...
				if (e.getEffectType() == EffectType.SHEAR)
				{
							...
							&& e.checkMob(entity))
					{
						...
						if (!sheep.isSheared())
						{
							if (sheep.isAdult())
							{
								...
								ItemStack item = e.getOutput(dcPlayer, sheep
										.getColor().getWoolData(), -1);
							...
							&& e.checkMob(entity))
					{
						...
						if (mooshroom.isAdult())
						{
							...
							Entity newE = entity.getWorld().spawnEntity(
									entity.getLocation(), EntityType.COW);
							Cow cow = (Cow) newE;
							...
							cow.setCustomNameVisible(mooshroom
									.isCustomNameVisible());
			...
			if (item.getType() == Material.RAW_FISH)
			{
				for (Skill skill : player.getSkills().values())
				{
					for (Effect effect : skill.getEffects())
					{
						if (effect.getEffectType() == EffectType.FISH)
						{
							...
							DwarfCraftEffectEvent ev = new DwarfCraftEffectEvent(
									player, effect, new ItemStack[] { item },
									new ItemStack[] { drop }, null, null,
									null, null, null, null, tool);
							plugin.getServer().getPluginManager()
									.callEvent(ev);
				...
				if (tool != null && tool.getType().getMaxDurability() > 0)
				{
					for (Skill s : player.getSkills().values())
					{
						for (Effect e : s.getEffects())
						{
							if (e.getEffectType() == EffectType.RODDURABILITY
									&& e.checkTool(tool))
