...
public class DCEntityListener implements Listener
{
		...
		if (trainer != null)
		{
			if (event.getClicker() instanceof Player)
			{
				...
				} else
				{
					...
					Skill skill = dCPlayer
							.getSkill(trainer.getSkillTrained());
		...
		try
		{
			...
			if (trainer != null)
			{
				...
				} else
				{
					...
					{
						plugin.getOut().sendMessage(dCPlayer.getPlayer(),
								"&6Please wait, Currently training a skill.");
					} else
					{
						long currentTime = System.currentTimeMillis();
						if ((currentTime - trainer.getLastTrain()) < (long) (plugin
								.getConfigManager().getTrainDelay() * 1000))
						{
							plugin.getOut().sendMessage(dCPlayer.getPlayer(),
									"&6Sorry, i need time to recuperate.");
							return true;
						} else
						{
							...
							plugin.getServer()
									...
									.scheduleSyncDelayedTask(
											plugin,
											new TrainSkillSchedule(trainer,
													dCPlayer), 2);
						}
					}
				}
	...
	public void onEntityAttack(EntityDamageByEntityEvent event)
	{
		...
		if ((plugin.getDataManager().isTrainer(event.getEntity()))
		...
		{// EvP no effects, EvE no effects
			if (DwarfCraft.debugMessagesThreshold < 4)
				System.out.println(String.format(
						"DC4: EVP %s attacked %s for %lf of %d\r\n", damager
								.getClass().getSimpleName(), victim
								.getClass().getSimpleName(), damage, hp));
		...
		ItemStack tool = attacker.getPlayer().getInventory()
				.getItemInMainHand();
		...
		for (Skill s : skills.values())
		{
			for (Effect e : s.getEffects())
			{
						...
						&& e.checkTool(tool))
				{
					...
					damage = plugin.getUtil().randomAmount(
							(e.getEffectAmount(attacker)) * damage);
					...
					DwarfCraftEffectEvent ev = new DwarfCraftEffectEvent(
							attacker, e, null, null, null, null, Origdamage,
							damage, victim, null, tool);
					...
					{
						System.out.println(String.format(
								"DC6: PVE %s attacked %s for %.2f of %d doing %lf dmg of %lf hp"
										+ " effect called: %d", attacker
										.getPlayer().getName(), victim
										.getClass().getSimpleName(), e
										.getEffectAmount(attacker), event
										.getDamage(), damage, hp, e.getId()));
					}
				}
				
				if (e.getEffectType() == EffectType.PVPDAMAGE && isPVP
						&& e.checkTool(tool))
				{
					damage = plugin.getUtil().randomAmount(
							(e.getEffectAmount(attacker)) * damage);
					...
					{
						System.out.println(String.format(
								"DC6: PVP %s attacked %s for %.2f of %d doing %lf dmg of %lf hp"
										+ " effect called: %d", attacker
										.getPlayer().getName(),
								((Player) victim).getName(), e
										.getEffectAmount(attacker), event
										.getDamage(), damage, hp, e.getId()));
					}
				}
	...
	public void onEntityDamageByProjectile(EntityDamageByEntityEvent event)
	{
			...
			for (Skill skill : attackDwarf.getSkills().values())
			{
				for (Effect effect : skill.getEffects())
				{
					if (effect.getEffectType() == EffectType.BOWATTACK)
					{
						...
						DwarfCraftEffectEvent ev = new DwarfCraftEffectEvent(
								attackDwarf, effect, null, null, null, null,
								origDamage, damage, hitThing, null, null);
						plugin.getServer().getPluginManager().callEvent(ev);
		...
		damage = plugin.getUtil().randomAmount(
				(damage * mitigation) + (origDamage / 4));
		event.setDamage(damage);
		if (damage >= hp && attacker instanceof Player
				&& !killMap.containsKey(hitThing)
				&& !(hitThing instanceof Player))
		{
	...
	public void onEntityDamagedByEnvirons(EntityDamageEvent event)
	{
		...
		if ((event.getEntity() instanceof Player))
		{
			...
			for (Skill s : dCPlayer.getSkills().values())
			{
				for (Effect e : s.getEffects())
				{
					if (e.getEffectType() == EffectType.FALLDAMAGE
							&& event.getCause() == DamageCause.FALL)
						damage = plugin.getUtil().randomAmount(
								e.getEffectAmount(dCPlayer) * damage);
					else if (e.getEffectType() == EffectType.FIREDAMAGE
							&& event.getCause() == DamageCause.FIRE)
						damage = plugin.getUtil().randomAmount(
								e.getEffectAmount(dCPlayer) * damage);
					else if (e.getEffectType() == EffectType.FIREDAMAGE
						...
						damage = plugin.getUtil().randomAmount(
								e.getEffectAmount(dCPlayer) * damage);
					else if (e.getEffectType() == EffectType.EXPLOSIONDAMAGE
							&& event.getCause() == DamageCause.ENTITY_EXPLOSION)
						damage = plugin.getUtil().randomAmount(
								e.getEffectAmount(dCPlayer) * damage);
					else if (e.getEffectType() == EffectType.EXPLOSIONDAMAGE
							...
							&& event.getCause() == DamageCause.FALL)
					{
						if (event.getDamage() <= e.getEffectAmount(dCPlayer))
						{
							if (DwarfCraft.debugMessagesThreshold < 1)
								System.out
										.println("DC1: Damage less than fall threshold");
							event.setCancelled(true);
						}
					}
					
					DwarfCraftEffectEvent ev = new DwarfCraftEffectEvent(
							dCPlayer, e, null, null, null, null, origDamage,
			...
			if (DwarfCraft.debugMessagesThreshold < 1)
			{
				System.out
						.println(String
								.format("DC1: environment damage type: %s base damage: %lf new damage: %.2lf\r\n",
					...
					if (effect.getEffectType() == EffectType.MOBDROP)
					{
						if (effect.checkMob(deadThing))
						{
							...
							if (deadThing instanceof CraftSheep)
								output.setDurability((short) ((CraftSheep) deadThing)
										.getColor().ordinal());
							...
							if (DwarfCraft.debugMessagesThreshold < 5)
							{
								System.out
										.println(String
												.format("DC5: killed a %s effect called: %d created %d of %s\r\n",
														deadThing
																.getClass()
																.getSimpleName(),
														effect.getId(),
						...
						{
							DwarfCraftEffectEvent ev = new DwarfCraftEffectEvent(
									killer, effect, normal, items.toArray(
											new ItemStack[items.size()])
