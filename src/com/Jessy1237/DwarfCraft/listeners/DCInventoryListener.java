...
public class DCInventoryListener implements Listener
{
	...
	public void onFurnaceExtractEvent(FurnaceExtractEvent event)
	{
		...
		for (Skill s : skills.values())
		{
			...
			{
				if (effect.getEffectType() == EffectType.SMELT
						&& effect.checkInitiator(new ItemStack(item)))
				{
					item = effect.getOutput().getType();
					int newAmount = plugin.getUtil().randomAmount(
							amount * effect.getEffectAmount(player));
					
					DwarfCraftEffectEvent ev = new DwarfCraftEffectEvent(
							player,
							effect,
							new ItemStack[] {},
							new ItemStack[] { new ItemStack(item, newAmount,
									effect.getOutput().getData().getData()) },
							null, null, null, null, null, event.getBlock(),
							null);
		...
		if (player != null && hasItems(toCraft))
		{
			...
			if (plugin.getUtil().isTool(toCraft.getTypeId()))
			{
				...
				if (ci.getRecipe() instanceof ShapelessRecipe)
				{
					...
					{
						if (plugin.getUtil().isTool(i.getTypeId())
								&& toCraft.getTypeId() == i.getTypeId())
						{
							return;
						}
					}
				...
				for (Skill s : dCPlayer.getSkills().values())
				{
					for (Effect e : s.getEffects())
					{
						if (e.getEffectType() == EffectType.CRAFT
								&& e.checkInitiator(toCraft.getTypeId(),
										(byte) toCraft.getData().getData()))
						{
							...
							for (ItemStack i : player.getInventory()
									.all(toCraft.getType()).values())
							{
								held += i.getAmount();
							}
							
							final ItemStack output = e.getOutput(dCPlayer,
									(byte) toCraft.getData().getData());
							
							float modifier = (float) output.getAmount()
									/ (float) toCraft.getAmount();
							...
							DwarfCraftEffectEvent ev = new DwarfCraftEffectEvent(
									dCPlayer,
									e,
									new ItemStack[] { check != null ? new ItemStack(
											output.getType(), 0, output
													.getData().getData())
											: toCraft },
									new ItemStack[] { output }, null, null,
				...
				if (isStackSumLegal(toCraft, toStore))
				{
					...
					for (Skill s : dCPlayer.getSkills().values())
					{
						for (Effect e : s.getEffects())
						{
							if (e.getEffectType() == EffectType.CRAFT
									&& e.checkInitiator(toCraft.getTypeId(),
											(byte) toCraft.getData()
													.getData()))
							{
								
								final ItemStack output = e.getOutput(
										dCPlayer, (byte) toCraft.getData()
												.getData());
								
								DwarfCraftEffectEvent ev = new DwarfCraftEffectEvent(
										dCPlayer, e,
										new ItemStack[] { toCraft },
										new ItemStack[] { output }, null,
								...
								for (ItemStack item : ev.getAlteredItems())
								{
									if (item != null)
									{
										if (item.getAmount() > 0)
										{
											if (item.getType() == toCraft
													.getType())
											{
												toCraft.setAmount(item
														.getAmount());
	...
	private boolean isStackSumLegal(ItemStack a, ItemStack b)
	{
		...
		if (a == null || b == null)
			...
			return a.getAmount() + b.getAmount() <= a.getType()
					.getMaxStackSize();
	}
	...
	private void handleShiftClickFurnace(InventoryClickEvent event)
	{
		if (event.isShiftClick())
		{
			...
			for (Skill s : skills.values())
			{
				...
				{
					if (effect.getEffectType() == EffectType.SMELT
							&& effect.checkInitiator(extract))
					{
						
						int held = 0;
						for (ItemStack i : player.getInventory()
								.all(extract.getType()).values())
						{
							held += i.getAmount();
						}
						
						ItemStack output = effect.getOutput(dCPlayer,
								(byte) extract.getData().getData());
						...
						DwarfCraftEffectEvent ev = new DwarfCraftEffectEvent(
								...
								effect,
								new ItemStack[] { check != null ? new ItemStack(
										output.getType(), 0, output.getData()
												.getData()) : extract },
								new ItemStack[] { output }, null, null, null,
								null, null,
								((BlockState) event.getInventory()
	...
	{
		if (!plugin.getUtil()
				.isWorldAllowed(event.getWhoClicked().getWorld()))
			return;
		...
		if (event.getSlotType() == SlotType.CRAFTING
				...
				&& event.getInventory().getHolder() instanceof BrewingStand)
		{
			DCPlayer player = plugin.getDataManager().find(
					(Player) event.getWhoClicked());
			HashMap<Integer, Skill> skills = player.getSkills();
			ItemStack item = event.getCurrentItem();
			final int amount = item.getAmount();
			BrewingStand block = (BrewingStand) event.getInventory()
					.getHolder();
			...
			if (stack != null)
			{
				if (sameInv(stack, block.getInventory()))
				{
					for (Skill s : skills.values())
					{
						for (Effect effect : s.getEffects())
						{
							if (effect.getEffectType() == EffectType.BREW
									&& effect.checkInitiator(item))
							{
								int newAmount = (int) (amount * effect
										.getEffectAmount(player));
								
								DwarfCraftEffectEvent ev = new DwarfCraftEffectEvent(
										player, effect,
										new ItemStack[] { item },
										new ItemStack[] { new ItemStack(item
												.getType(), newAmount, item
												.getData().getData()) },
										null, null, null, null, null,
										block.getBlock(), null);
								plugin.getServer().getPluginManager()
	...
	public boolean sameInv(ItemStack[] orig, BrewerInventory new1)
	{
		for (int n = 0; n != orig.length; n++)
		{
			...
			if (i != null)
			{
				...
				{
					if (new1.getItem(n).getData().getData() == i.getData()
							.getData())
					{
						return true;
					}
	...
	public BrewerInventory check(Location l)
	{
		for (Location l1 : stands.keySet())
		{
			if (l1 != null)
			{
				if (l1.getX() == l.getX() && l1.getY() == l.getY()
						&& l1.getZ() == l.getZ())
				{
...
class ShiftClickTask implements Runnable
{
	...
	public ShiftClickTask(DwarfCraft plugin, DCPlayer p,
			final ItemStack item, ItemStack check, int init, float modifier,
			Effect e)
	{
		this.p = p;
		this.item = item;
	...
	public void run()
	{
		...
		Collection<ItemStack> items = (Collection<ItemStack>) p.getPlayer()
				.getInventory().all(check.getType()).values();
		...
		if (modifier > 1)
		{
			final int amount = plugin.getUtil().randomAmount(
					(modifier * difference - difference));
			...
			for (int i = amount; i > 0; i -= item.getMaxStackSize())
			{
				if (i > item.getMaxStackSize())
				{
					p.getPlayer()
							...
							.dropItemNaturally(
									...
									new ItemStack(item.getType(), item
				...
				} else
				{
					p.getPlayer()
							...
							.dropItemNaturally(
									p.getPlayer().getLocation(),
									new ItemStack(item.getType(), i, item
											.getDurability()));
				}
			}
		...
		{
			int amount = plugin.getUtil().randomAmount(
					(difference - modifier * difference));
			p.getPlayer()
					.getInventory()
					.removeItem(
							new ItemStack(item.getType(), amount, item
									.getDurability()));
		}
