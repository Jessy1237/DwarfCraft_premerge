...
public final class CommandParser
{
	...
	public CommandParser(final DwarfCraft plugin, CommandSender sender,
			String[] args)
	{
		this.plugin = plugin;
		this.sender = sender;
			...
			for (Object o : desiredArguments)
			{
				if (o instanceof DCPlayer)
					...
					output.add(parseGreeterMessage(arrayIterator));
						...
						&& ((String) o).equalsIgnoreCase("Type"))
	...
	private DCPlayer parseDwarf(int argNumber) throws DCCommandException
	{
		...
		try
		{
			...
			else if (player == null || !player.isOnline())
			{
				...
				dCPlayer = plugin.getDataManager().findOffline(
						plugin.getServer().getOfflinePlayer(dwarf)
								.getUniqueId());
			}
			if (dCPlayer == null)
			{
	...
	private String parseGreeterMessage(int arrayIterator)
	...
	private Skill parseSkill(int argNumber) throws DCCommandException
	{
		...
		if (!(sender instanceof Player))
		{
			...
			} catch (NumberFormatException nfe)
			{
				...
				for (int i : plugin.getConfigManager().getAllSkills(
						target.getRace()))
				{
					Skill s = plugin.getConfigManager().getGenericSkill(i);
					if (s.getDisplayName()
							.regionMatches(0, inputString, 0, 8))
						return s;
				}
			}
			throw new DCCommandException(plugin, Type.PARSESKILLFAIL);
