 ...
 * weapon damage, decreased tool durability drop, increased drops from blocks
 * or mobs, etc.
 * 
 * Data used for this plugin comes from two places: On each load, a list of
 * skills and effects is pulled from flatfiles. Dwarf's skill levels
...
public class DwarfCraft extends JavaPlugin
{
	...
	private final DCVehicleListener		vehicleListener			= new DCVehicleListener(
																		this);
	private final DCInventoryListener	inventoryListener		= new DCInventoryListener(
																		this);
	private final DCListener			dcListener				= new DCListener(
																		this);
			...
			String type)
	{
		...
		if (sender instanceof Player)
		{
			if (type.equals("op"))
			{
				return perms.has((Player) sender,
						("DwarfCraft.op." + name).toLowerCase());
			} else if (type.equals("norm"))
			{
				return perms.has((Player) sender,
						("DwarfCraft.norm." + name).toLowerCase());
			} else if (type.equals("all"))
			{
				return perms.has((Player) sender,
						"DwarfCraft.*".toLowerCase());
			}
		}
	...
	public void onEnable()
	{
		...
		if (pm.getPlugin("Vault") == null
				|| pm.getPlugin("Vault").isEnabled() == false)
		{
		...
		{
			System.out
					.println("[DwarfCraft] Unable to find a permissions plugin.");
			pm.disablePlugin(this);
			return;
		}
		...
		if (pm.getPlugin("Citizens") == null
				|| pm.getPlugin("Citizens").isEnabled() == false)
		{
		...
		CitizensAPI.getTraitFactory().registerTrait(
				TraitInfo.create(DwarfTrainerTrait.class).withName(
						"DwarfTrainer"));
		...
		System.out.println("[DwarfCraft]" + getDescription().getName()
