			...
			String[] args)
	{
		...
		if (args.length == 0)
		{
			plugin.getOut().sendMessage(sender,
					CommandInformation.Usage.SETSKILL.getUsage());
		} else if (args[0].equalsIgnoreCase("?"))
		{
			plugin.getOut().sendMessage(sender,
					CommandInformation.Desc.SETSKILL.getDesc());
		} else
		{
			try
			{
				...
				if (skill == null)
				{
					Skill[] skills = new Skill[dCPlayer.getSkills().values()
							.size()];
					int i = 0;
					...
					plugin.getOut().sendMessage(
							sender,
							"&aAdmin: &eset all skills for player &9" + name
									+ "&e to &3" + level);
			...
			{
				e.describe(sender);
				sender.sendMessage(CommandInformation.Usage.SETSKILL
						.getUsage());
				return false;
			}
		}
