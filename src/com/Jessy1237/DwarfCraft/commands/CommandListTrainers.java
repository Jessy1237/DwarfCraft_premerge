...
public class CommandListTrainers extends Command
{
			...
			String[] args)
	{
		...
		Collection<DwarfTrainer> col = plugin.getDataManager().trainerList
				.values();
		...
		Collection<Skill> skills = plugin.getConfigManager().getAllSkills()
				.values();
		...
		sender.sendMessage(String
				.format("Trainers page %d/%d", page, maxpage));
		...
		for (int x = 0; x < 10; x++)
		{
			...
			if (trainer.isGreeter())
			{
				sender.sendMessage(String.format(
						"Greeter ID: %s Name: %s (%d, %d, %d)",
						trainer.getUniqueId(), trainer.getName(),
			...
			} else
			{
				...
				sender.sendMessage(String.format(
						"Trainer ID: %s Name: %s Trains: %d %s (%d, %d, %d)",
						trainer.getUniqueId(), trainer.getName(),
						trainer.getMaxSkill(), skillName, loc.getBlockX(),
