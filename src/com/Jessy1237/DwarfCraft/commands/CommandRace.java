...
public class CommandRace extends Command
{
			...
			String[] args)
	{
		...
		} else if ((!(sender instanceof Player) || plugin.getPermission()
				.has(sender, "dwarfcraft.op.race")) && args.length == 1)
		{
			Player p = plugin.getServer().getPlayer(args[0]);
			if (p == null)
			{
				plugin.getOut().sendMessage(sender,
						CommandInformation.Usage.RACE.getUsage());
			} else
			{
		...
		{
			plugin.getOut().sendMessage(sender,
					CommandInformation.Desc.RACE.getDesc());
		} else if (args.length == 3)
		{
			...
			if (sender instanceof Player)
			{
				...
				{
					race(newRace, confirmed, dCPlayer, (CommandSender) plugin
	...
	private void race(String newRace, boolean confirm, DCPlayer dCPlayer,
		...
		} else
		{
			if (confirm)
			{
				if (plugin.getConfigManager().getRace(newRace) != null)
				{
					if (sender instanceof Player)
					{
						if (plugin.getPermission().has(
								(Player) sender,
								"dwarfcraft.norm.race."
										+ newRace.toLowerCase()))
						{
							plugin.getOut().changedRace(
									sender,
									dCPlayer,
									plugin.getConfigManager()
											.getRace(newRace).getName());
					...
					} else
					{
						plugin.getOut().changedRace(
								dCPlayer.getPlayer(),
								dCPlayer,
								plugin.getConfigManager().getRace(newRace)
										.getName());
						dCPlayer.changeRace(newRace);
					}
				} else
