			...
			String[] args)
	{
		...
		if (args.length == 0)
		{
			plugin.getOut().sendMessage(sender,
					CommandInformation.Usage.CREATEGREETER.getUsage());
		} else if (args[0].equalsIgnoreCase("?"))
		{
			plugin.getOut().sendMessage(sender,
					CommandInformation.Desc.CREATEGREETER.getDesc());
		} else
		{
			try
			{
				...
				npc.addTrait(new DwarfTrainerTrait(plugin, Integer
						.parseInt(uniqueId), null, null, null, true,
						greeterMessage));
				npc.setProtected(true);
			} catch (DCCommandException e)
			{
				e.describe(sender);
				sender.sendMessage(CommandInformation.Usage.CREATEGREETER
						.getUsage());
				return false;
			}
		}
