...
public class Out
{
	...
	public enum Color
	{
		DC(FixedColor.Gold), COMMAND(FixedColor.DarkRed), HELP(
				FixedColor.Pink), BADAMOUNT(FixedColor.Red), GOODAMOUNT(
				FixedColor.Green), NORMALAMOUNT(FixedColor.Yellow), ITEM(
				FixedColor.DarkGreen), SKILLID(FixedColor.DarkTeal), SKILLLEVEL(
				FixedColor.Teal), EFFECT(FixedColor.Purple), DWARF(
				FixedColor.Blue), ELF(FixedColor.White), TRAINER(
				FixedColor.Gray);
		FixedColor	fcolor;
	...
	{
		Black("§0"), DarkBlue("§1"), DarkGreen("§2"), DarkTeal("§3"), DarkRed(
				"§4"), Purple("§5"), Gold("§6"), Gray("§7"), DarkGray("§8"), Blue(
				"§9"), Green("§a"), Teal("§b"), Red("§c"), Pink("§d"), Yellow(
				"§e"), White("§f");
			...
			Effect effect)
	{
		sendMessage(sender, effect.describeLevel(dCPlayer),
				"&6[&5" + effect.getId() + "&6] ");
		sendMessage(sender, effect.describeGeneral(dCPlayer), "&6[&5"
				+ effect.getId() + "&6] ");
		return true;
	}
	...
	private String playerLinePrinter(Player player, String message,
			String prefix)
	{
		int messageSectionLength = lineLength
				- plugin.getUtil().msgLength(prefix);
		String currentLine = "";
		...
		{
			if (plugin.getUtil().msgLength(currentLine)
					+ plugin.getUtil().msgLength(word) <= messageSectionLength)
			{
	...
	public boolean printSkillInfo(CommandSender sender, Skill skill,
			DCPlayer dCPlayer, int maxTrainLevel)
	{
		// general line
		sendMessage(sender, String.format(
				"&6  Skillinfo for &b%s&6 [&b%d&6] || Your level &3%d/%d",
				skill.getDisplayName(), skill.getId(), skill.getLevel(),
				maxTrainLevel));
		...
		for (Effect effect : skill.getEffects())
		{
			if (effect != null)
				sendMessage(sender, effect.describeLevel(dCPlayer),
						String.format("&6[&5%d&6] ", effect.getId()));
		}
		...
		sendMessage(
				sender,
				String.format("&6---Train costs for level &3%d",
						(skill.getLevel() + 1)));
		List<List<ItemStack>> costsTurnins = dCPlayer
		...
		for (int i = 0; i < remaining.size(); i++)
		{
			...
			if (r != null && t != null)
			{
				...
				sendMessage(sender, String.format(" &2%d of %d %s&6  --",
						deposited, totalCost, plugin.getUtil()
								.getCleanName(r)), " &6-- ");
			}
			...
			String displayName, boolean printFull)
	{
		...
		message1 = ("&6Printing Skill Sheet for &9"
				+ (displayName == null ? dCPlayer.getPlayer().getName()
						: displayName) + " " + dCPlayer.getRace()
				+ " &6Level is &3" + dCPlayer.getDwarfLevel());
		...
		for (Skill s : dCPlayer.getSkills().values())
		{
			...
			String interim = String.format("&6[&3%02d&6] &b%.18s",
					s.getLevel(), s.getDisplayName());
	...
	{
		Player[] playerArray = server.getOnlinePlayers().toArray(
				new Player[server.getOnlinePlayers().size()]);
	...
	private String stripColors(String message)
	{
		if (message == null)
		{
			if (DwarfCraft.debugMessagesThreshold < 2)
				System.out
						.println("DC2: stripping colors from null message!");
			return null;
		}
		if (DwarfCraft.debugMessagesThreshold < -1)
	...
	public void welcome(Server server, DCPlayer dCPlayer)
	{
		try
		{
			if (plugin.getConfigManager().sendGreeting)
				sendBroadcast(server, "&fWelcome, &9" + dCPlayer.getRace()
						+ " &6" + dCPlayer.getPlayer().getName(),
						"&6[DC]         ");
		} catch (Exception e)
		{
			e.printStackTrace();
	...
	{
		sendMessage(sender,
				"You are a " + plugin.getDataManager().find(player).getRace());
	}
	
	public void adminRace(CommandSender sender, Player player)
	{
		sendMessage(sender, player.getDisplayName() + " is a "
				+ plugin.getDataManager().find(player).getRace());
	}
	...
	public void resetRace(CommandSender sender, DCPlayer dCPlayer,
			String newRace)
	{
			...
			String newRace)
	{
		sendMessage(
				sender,
				"You need to confirm this command with confirm at the end. (Note: This will reset all your skills)");
	}
	
	public void dExistRace(CommandSender sender, DCPlayer dCPlayer,
			String newRace)
	{
