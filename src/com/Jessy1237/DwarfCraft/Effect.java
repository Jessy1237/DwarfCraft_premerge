...
public class Effect
{
	...
	public Effect(CSVRecord record, DwarfCraft plugin)
	{
		...
		{
			mInitiator = plugin.getUtil().parseItem(
					record.getString("OriginID"));
		} else
		{
	...
	protected String describeLevel(DCPlayer dCPlayer)
	{
		...
		switch (mType)
		{
			...
			case MOBDROP:
				...
				{
					description = String.format(
							"&6%s drop about %s%.2f &2%s", plugin.getUtil()
									.getCleanName(mCreature),
							effectLevelColor, effectAmount, output);
					break;
				}
				description = String.format(
						"&6Enemies that drop &2%s &6leave about %s%.2f&6",
			...
			case EXPLOSIONDAMAGE:
				...
				{
					description = String
							.format("&6You take %s%d%% more &6damage from explosions",
									effectLevelColor,
				...
				{
					description = String
							.format("&6You take %s%d%% less &6damage from explosions",
									effectLevelColor,
									(int) (effectAmount * 100 - 100));
					break;
				}
			...
			case FALLTHRESHOLD:
				description = String
						.format("&6Fall damage less than %s%d &6does not affect you.",
			...
			case RODDURABILITY:
				description = String.format(
						"&6Using a &2%s &6removes about %s%.2f &6durability",
						toolType, effectLevelColor, effectAmount);
				break;
			case EAT:
				description = String
						.format("&6You gain %s%.2f &6Hunger instead of &e%.2f&6 when you eat &2%s",
								effectLevelColor, effectAmount / 2.0,
								((double) Util.FoodLevel.getLvl(mInitiator
										.getTypeId())) / 2.0, initiator);
				break;
			case CRAFT:
				description = String.format(
			...
			case FISH:
				description = String
						.format("&6You catch %s%.2f &6fish instead of &e%.2f &6when you fish",
								effectLevelColor, effectAmount, elfAmount);
				break;
			case BREW:
				description = String
						.format("&6You brew %s%.2f &6potion(s) instead of &e%.2f &6when you're brewing potions",
								effectLevelColor, effectAmount, elfAmount);
				break;
			case DIGTIME:
				description = String
						.format("&a%.0f%%&6 of the time &2%s &6break &2%s &6instantly ",
								effectAmount * 100, toolType, initiator);
				break;
			case BOWATTACK:
				description = String
						.format("&6Your Arrows (Fully Charge Bow) do %s%.0f &6hp damage (half hearts)",
								effectLevelColor, effectAmount + 2);
				break;
			case VEHICLEDROP:
				description = String
						.format("&6DISABLED: When you break a boat &6approx. %s%.2f &2%s&6 are created",
								effectLevelColor, effectAmount, output);
				break;
			case VEHICLEMOVE:
				description = String.format(
			...
			case SMELT:
				...
				{
					initiator = plugin.getUtil().getCleanName(
				...
				description = String
						.format("&6Smelt a &2%s &6and %s%.2f &2%s&6 are created as well",
								initiator, effectLevelColor, effectAmount,
								output);
				break;
			case SHEAR:
				description = String
						.format("&6Shear a %s and %s%.2f &6%s are dropped instead of &e%.2f",
								plugin.getUtil().getCleanName(mCreature),
								effectLevelColor, effectAmount, output,
								elfAmount);
				break;
			case SPECIAL:
	...
	{
		return getEffectAmount(dCPlayer.getSkillLevel(this.mID / 10),
				dCPlayer);
	}
	...
	public double getEffectAmount(int skillLevel, DCPlayer dCPlayer)
	{
		...
		if (dCPlayer != null)
			if (mException
					&& skillLevel <= mExceptionHigh
					&& skillLevel >= mExceptionLow
					&& !(skillLevel == plugin.getConfigManager()
							.getRaceLevelLimit() && plugin
							.getConfigManager()
							.getAllSkills(dCPlayer.getRace())
							.contains(
									plugin.getConfigManager().getAllSkills()
											.get(mID / 10))))
		...
		if (DwarfCraft.debugMessagesThreshold < 1)
		{
			System.out
					.println(String
							.format("DC1: GetEffectAmmount ID: %d Level: %d Base: %.2f Increase: %.2f Novice: %.2f Max: %.2f Min: %.2f "
									+ "Exception: %s Exctpion Low: %.2f Exception High: %.2f Exception Value: %.2f Floor Result: %s",
									mID, skillLevel, mBase, mLevelIncrease,
									mLevelIncreaseNovice, mMax, mMin,
									mException, mExceptionLow,
									mExceptionHigh, mExceptionValue,
									mFloorResult));
		}
	...
	{
		Byte data = (mOutput.getData() == null ? null : mOutput.getData()
				.getData());
		
		if (data != null && data == 0)
			data = oldData;
		
		final int count = plugin.getUtil().randomAmount(
				getEffectAmount(player));
		ItemStack item = null;
		if (plugin.getUtil().checkEquivalentBuildBlocks(mOutput.getTypeId(),
				oldID) == null
				|| oldID == -1)
		{
	...
	public boolean checkInitiator(ItemStack item)
	{
		if (item == null)
			...
			return checkInitiator(item.getTypeId(),
	...
	public boolean checkInitiator(int id, byte data)
	{
		if (mInitiator.getTypeId() != id
				&& plugin.getUtil().checkEquivalentBuildBlocks(id,
						mInitiator.getTypeId()) == null)
			return false;
	...
	public void damageTool(DCPlayer player, int base, ItemStack tool,
			boolean negate)
	{
		short wear = (short) (plugin.getUtil().randomAmount(
				getEffectAmount(player)) * base);
		
		if (DwarfCraft.debugMessagesThreshold < 2)
			System.out
					.println(String
							.format("DC2: Affected durability of a \"%s\" - Effect: %d Old: %d Base: %d Wear: %d",
									plugin.getUtil().getCleanName(tool), mID,
									tool.getDurability(), base, wear));
		...
		DwarfCraftEffectEvent e = new DwarfCraftEffectEvent(player, this,
				null, null, null, null, (double) base, (double) wear, null,
				null, tool);
		...
		tool.setDurability((short) (tool.getDurability()
				+ e.getAlteredDamage() - base));
