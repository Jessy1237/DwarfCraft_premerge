...
public class DwarfTrainerTrait extends Trait{
	...
	@Persist(required=true) private int mSkillID;
	@Persist(required=true) private int mMaxLevel;
	@Persist(required=true) private int mMinLevel;
	@Persist(required=true) private boolean mIsGreeter;
	@Persist(required=true) private String mMsgID;
	...
	public void onSpawn() {
		...
		if (this.mHeldItem != Material.AIR)
			((LivingEntity) getNPC().getEntity()).getEquipment().setItemInHand(new ItemStack(mHeldItem, 1));
